...
from mark_i.ui.gui.gui_utils import validate_and_get_widget_value  # create_clickable_list_item is now used by CEC
...
class DetailsPanel(ctk.CTkScrollableFrame):
    def __init__(self, master: Any, parent_app: Any, **kwargs):
        ...
        # self.selected_sub_condition_item_widget: Optional[ctk.CTkFrame] = None # Now managed by CEC visually
    ...
    def clear_content(self):
        ...
        # self.selected_sub_condition_item_widget = None # CEC handles its own visual selection
    ...
    def _display_rule_details(self, rule_data: Dict[str, Any]):
        ...
        current_master_row += 1
        self.condition_editor_component_instance.update_ui_with_condition_data(cond_data_for_editor)  # Pass data to CEC
    ...
    # This method (_render_rule_condition_editor_internal) is now OBSOLETE
    # Its logic has been integrated into ConditionEditorComponent.update_ui_with_condition_data
    # and ConditionEditorComponent's internal _render_single/compound methods.
    # DetailsPanel calls CEC.update_ui_with_condition_data.

    # This method (_populate_sub_conditions_list_internal) is now OBSOLETE
    # CEC._populate_sub_conditions_list handles this internally.

    # This method (_on_sub_condition_selected_internal) is now OBSOLETE
    # CEC._on_sub_condition_selected handles this, then calls _cec_render_sub_condition_params_editor on DP.

    def _cec_render_sub_condition_params_editor(self, sub_cond_data: Dict[str, Any], index: int):
        """
        Called by ConditionEditorComponent when one of its sub-conditions is selected.
        This method renders the parameter editor for that sub-condition into
        the ConditionEditorComponent's designated frame.
        """
        ...
        for w in target_editor_frame.winfo_children():  # Clear previous editor
            w.destroy()
        ...
        type_menu = ctk.CTkOptionMenu(
            ...
            command=lambda choice: self.parent_app._on_rule_part_type_change("condition", choice),  # This still calls MainApp to trigger full re-render of CEC
        )
    ...
    def _cec_convert_condition_structure(self):
        logger.debug("DP: _cec_convert_condition_structure called, dispatching to parent_app.")
        self.parent_app._convert_condition_structure()

    def _cec_add_sub_condition(self):
        logger.debug("DP: _cec_add_sub_condition called, dispatching to parent_app.")
        self.parent_app._add_sub_condition_to_rule()

    def _cec_remove_selected_sub_condition(self):
        logger.debug("DP: _cec_remove_selected_sub_condition called, dispatching to parent_app.")
        self.parent_app._remove_selected_sub_condition()

    def _cec_on_condition_type_change(self, part_key: str, new_type: str):
        """Called by CEC when a condition/sub-condition type changes."""
        logger.debug(f"DP: _cec_on_condition_type_change called for '{part_key}' to '{new_type}'. Dispatching to parent_app.")
        # This needs to be robust. MainAppWindow._on_rule_part_type_change
        # expects to update DetailsPanel, which now means DetailsPanel needs
        # to update its CEC instance.
    ...
    def _render_dynamic_parameters(self, param_group_key: str, item_subtype: str, data_source: Dict[str, Any], parent_frame: ctk.CTkFrame, start_row: int, widget_prefix: str):
        ...
        if not param_defs_for_subtype and item_subtype not in ["always_true"]:
            ctk.CTkLabel(parent_frame, text=f"No params for '{item_subtype}'.", text_color="gray").grid(row=current_r, column=0, columnspan=2, pady=5)
            return
    ...
    def get_all_rule_data_from_ui(self) -> Optional[Dict[str, Any]]:
        # Unchanged for now, still centralizes data gathering
    ...
    def _get_parameters_from_ui(self, param_group_key: str, item_subtype: str, widget_prefix: str) -> Optional[Dict[str, Any]]:
        ...
        for p_def in param_defs:
            ...
            if widget is None and not isinstance(tk_var, tk.BooleanVar):
                if eff_req:
                    logger.error(f"DP: Widget for required '{lbl_err}' (ID: {p_id}) not found.")
                    all_ok = False
                params[p_id] = def_val
                continue
        ...
        if item_subtype == "always_true" and param_group_key == "conditions":
            ...
            if region_param_def_for_at:
                region_val_for_at, _ = validate_and_get_widget_value(
                    self.detail_widgets.get(f"{widget_prefix}region"), self.detail_optionmenu_vars.get(f"{widget_prefix}region_var"), "Region (always_true)", str, "", required=False
                )
                if region_val_for_at:
