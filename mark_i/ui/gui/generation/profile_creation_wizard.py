...
import numpy as np
from PIL import Image, ImageTk, ImageDraw, UnidentifiedImageError, ImageGrab  # Added ImageGrab
...
from mark_i.ui.gui.gui_config import CONDITION_TYPES, ACTION_TYPES, UI_PARAM_CONFIG, OPTIONS_CONST_MAP  # Changed from ALL_CONDITION_TYPES  # Assuming this is correct
...
class ProfileCreationWizardWindow(ctk.CTkToplevel):
    ...
    def __init__(self, master: Any, main_app_instance: Any):  # main_app_instance is MainAppWindow
        ...
        self.user_cancelled_wizard = False  # Flag for MainAppWindow if wizard is cancelled before saving
        self.newly_saved_profile_path: Optional[str] = None  # For MainAppWindow to load new profile
        ...
        self.profile_generator = ProfileGenerator(gemini_analyzer=self.main_app_instance.gemini_analyzer_instance, config_manager=self.profile_generator_cm)
    ...
    def _clear_main_content_frame(self):
        for widget in self.main_content_frame.winfo_children():
            widget.destroy()
        self.step_logic_detail_widgets.clear()
        self.step_logic_optionmenu_vars.clear()
        self.step_logic_param_widgets_and_defs.clear()
        self.step_logic_controlling_widgets.clear()
    ...
    def _show_current_page(self):
        ...
        if self.current_page_index == PAGE_GOAL_INPUT:
            self._setup_page_goal_input()
        elif self.current_page_index == PAGE_PLAN_REVIEW:
            self._setup_page_plan_review()
        elif self.current_page_index == PAGE_STEP_DEFINE_REGION:
            self._setup_page_step_define_region()
        elif self.current_page_index == PAGE_STEP_DEFINE_LOGIC:
            self._setup_page_step_define_logic()
        elif self.current_page_index == PAGE_FINAL_REVIEW_SAVE:
            self._setup_page_final_review_save()
        else:
            ctk.CTkLabel(self.main_content_frame, text=f"Error: Unknown page index {self.current_page_index}").pack()
    ...
    def _setup_page_goal_input(self):
        ...
        self.goal_textbox = ctk.CTkTextbox(page_frame, height=180, wrap="word", font=ctk.CTkFont(size=13))
        ...
        context_frame = ctk.CTkFrame(page_frame, fg_color="transparent")
        ...
        self.context_image_preview_label = ctk.CTkLabel(context_frame, text="No context image.", height=150, fg_color=("gray85", "gray25"), corner_radius=6)
        self.context_image_preview_label.pack(fill="x", pady=(10, 0))
        self._update_context_image_preview()
        self.goal_textbox.focus_set()
        logger.debug("Page Goal Input UI setup.")
    ...
    def _setup_page_plan_review(self):
        ...
        if self.intermediate_plan and len(self.intermediate_plan) > 0:
            ...
            plan_display_frame = ctk.CTkScrollableFrame(page_frame, height=450, fg_color=("gray92", "gray28"))
    ...
    def _setup_page_step_define_region(self):
        ...
        controls_frame = ctk.CTkFrame(page_frame, fg_color="transparent")
    ...
    def display_current_full_context_with_optional_overlay(self, label_widget: ctk.CTkLabel, overlay_box: Optional[List[int]] = None, box_color="red"):
        if not label_widget or not label_widget.winfo_exists():
            return
        if not self.current_full_context_pil:
    ...
    def _handle_draw_region_manually(self):
        ...
        self.focus_set()
        if not capture_pil_for_selector:
            messagebox.showerror("Capture Error", "Failed to get screen image for manual region selection.", parent=self)
            return
        ...
        initial_name_for_selector = (
            ...
            or self.current_step_region_name
            or (self._temp_suggested_region_coords and self.current_plan_step_data.get("suggested_region_name_hint"))
    ...
    def _confirm_and_add_current_step_region(self) -> bool:
        ...
        if not region_name_from_ui:
            messagebox.showerror("Input Error", "Region Name cannot be empty.", parent=self)
            self.current_step_region_name_entry.focus()
            return False
        if not self.current_step_region_coords:
            messagebox.showerror("Error", "No region coordinates defined/confirmed for this step.", parent=self)
            return False
        ...
        if any(r.get("name") == self.current_step_region_name for r in existing_regions_in_draft):
            if not messagebox.askyesno(
                "Name Conflict",
                f"A region named '{self.current_step_region_name}' already exists in the profile being generated. Its definition will be updated with the current coordinates if you proceed. Continue?",
                parent=self,
            ):
                self.current_step_region_name_entry.focus()
                return False
        ...
        if self.profile_generator.add_region_definition(region_data_to_add_to_pg):
            ...
            if self.current_full_context_np is not None and self.current_step_region_coords:
                ...
                if w_clamped > 0 and h_clamped > 0:
                    ...
                    self.current_step_region_image_np = None
                    self.current_step_region_image_pil_for_display = None
                    ...
                    messagebox.showerror("Crop Error", "Defined region resulted in invalid crop.", parent=self)
                    return False
            else:
                self.current_step_region_image_np = None
                self.current_step_region_image_pil_for_display = None
    ...
    def _setup_page_final_review_save(self):
        ...
        profile_summary_textbox = ctk.CTkTextbox(page_frame, height=450, wrap="word", state="disabled", font=ctk.CTkFont(family="Courier New", size=11))
        profile_summary_textbox.pack(fill="both", expand=True, pady=(0, 10))
        data = self.profile_generator.get_generated_profile_data()
        if self.user_goal_text:
            data["profile_description"] = f"AI-Generated for goal: {self.user_goal_text[:120]}"
        ...
        name_frame = ctk.CTkFrame(page_frame, fg_color="transparent")
        name_frame.pack(fill="x", pady=(10, 5))
        ctk.CTkLabel(name_frame, text="Profile Filename:").pack(side="left", padx=(0, 5))
        self.profile_filename_entry = ctk.CTkEntry(name_frame, placeholder_text=self.generated_profile_name_base)
        self.profile_filename_entry.pack(side="left", expand=True, fill="x")
        self.profile_filename_entry.insert(0, self.generated_profile_name_base)
    ...
    def _display_current_step_region_image_with_candidates(self, candidate_boxes: Optional[List[Dict[str, Any]]] = None, selected_box_idx: Optional[int] = None):
        if not hasattr(self, "step_logic_region_image_label") or not self.step_logic_region_image_label.winfo_exists():
            return
        if not self.current_step_region_image_pil_for_display:
            self.step_logic_region_image_label.configure(text="No image for current step's region.", image=None)
            return
        img_pil_to_draw_on = self.current_step_region_image_pil_for_display.copy()
    ...
    def _on_region_image_click_for_element_selection(self, event):
        if not self.ui_refined_element_candidates or not hasattr(self.step_logic_region_image_label, "photo_image_scale_factor_x"):
            return
        ...
        newly_selected_idx = next(
            (
                i
                for i, c in enumerate(self.ui_refined_element_candidates)
    ...
    def _update_action_params_with_selected_element(self):
        ...
        logger.info(
            f"Element confirmed for action: Label='{self.ui_confirmed_element_for_action['value']['element_label']}', Box={self.ui_confirmed_element_for_action['value']['box']} in region '{self.current_step_region_name}'"
        )
    ...
    def _attempt_set_template_name_in_ui(self, template_name_to_set: str):
        ...
        tk_var = self.step_logic_optionmenu_vars.get(f"{widget_key}_var")
        widget = self.step_logic_detail_widgets.get(widget_key)
        if tk_var and isinstance(tk_var, tk.StringVar) and widget and isinstance(widget, ctk.CTkOptionMenu):
            ...
            if template_name_to_set in new_opts:
                ...
                logger.warning(f"Newly added template '{template_name_to_set}' not found in dropdown options after repopulating. Options: {new_opts}")
        else:
            logger.warning(f"Could not find or set template_name dropdown for '{template_name_to_set}'.")
    ...
    def _set_click_action_params_for_template(self, template_name: str):
        ...
        if target_relation_var and isinstance(target_relation_var, tk.StringVar):
            ...
            logger.warning("Wizard: Could not find target_relation_var for click action to set for template.")
        gem_var_entry = self.step_logic_detail_widgets.get("step_act_gemini_element_variable")
        if gem_var_entry and isinstance(gem_var_entry, ctk.CTkEntry):
            gem_var_entry.delete(0, tk.END)
        self.ui_confirmed_element_for_action = None
    ...
    def _get_parameters_from_ui_wizard_scoped(self, param_group_key: str, item_subtype: str, widget_prefix: str) -> Optional[Dict[str, Any]]:
        ...
        for p_def in param_defs:
            ...
            widget = self.step_logic_detail_widgets.get(w_key)
            tk_var = self.step_logic_optionmenu_vars.get(f"{w_key}_var")
            ...
            if widget is None and not isinstance(tk_var, tk.BooleanVar):
                if eff_req:
                    logger.error(f"Wizard GetParams: UI Widget for required parameter '{lbl_err}' (ID: {p_id}) not found.")
                    all_ok = False
                params[p_id] = def_val
                continue
    ...
    def _get_current_step_logic_from_ui(self) -> Optional[Tuple[Dict[str, Any], Dict[str, Any]]]:
        ...
        if self.ui_confirmed_element_for_action and action_params.get("type") == "click" and action_params.get("gemini_element_variable") == self._ui_current_step_temp_var_name:
            ...
            if source_region_config_from_draft:
                ...
                action_params.pop("target_region", None)
    ...
    def _go_to_next_page(self):
        ...
        if self.current_page_index == PAGE_GOAL_INPUT:
            ...
            if not self.user_goal_text:
                messagebox.showerror("Input Error", "Please describe goal.", parent=self)
                return
            ...
            if self.profile_generator.advance_to_next_plan_step():
                ...
                messagebox.showerror("Error", "Plan empty or issue advancing.", parent=self)
                self.current_page_index = PAGE_GOAL_INPUT
            self._show_current_page()
        elif self.current_page_index == PAGE_STEP_DEFINE_REGION:
            if self._confirm_and_add_current_step_region():
            ...
            if logic_tuple:
                ...
                if current_step and self.current_step_region_name:
                    ...
                    rule_to_add = {
                        ...
                        "region": self.current_step_region_name,
                        "condition": confirmed_condition,
                        "action": confirmed_action,
                        "comment": f"AI Gen for: {current_step.get('description')}",
                    }
                    if self.profile_generator.add_rule_definition(rule_to_add):
                        logger.info(f"Rule '{rule_name_to_add}' added for step {current_step.get('step_id')}.")
                    else:
                        messagebox.showerror("Error", f"Failed to add rule for step {current_step.get('step_id')}.", parent=self)
                        return
                else:
                    messagebox.showerror("Internal Error", "Missing step data or region name when adding rule.", parent=self)
                    return
                if self.profile_generator.advance_to_next_plan_step():
                    self.current_page_index = PAGE_STEP_DEFINE_REGION
                    self._reset_and_load_step_state()
                else:
                    self.current_page_index = PAGE_FINAL_REVIEW_SAVE
                self._show_current_page()
            else:
                return
        elif self.current_page_index == PAGE_FINAL_REVIEW_SAVE:
            self._save_generated_profile()
            return
    ...
    def _handle_generate_plan_result(self, plan: Optional[IntermediatePlan], error: Optional[Exception]):
        ...
        if self.intermediate_plan and len(self.intermediate_plan) > 0:
            ...
            messagebox.showinfo(
                "AI Plan",
                "AI generated an empty plan. This might mean the goal was too vague or complex. You can refine your goal or proceed to build manually if desired (though the wizard will end after this if no steps).",
                parent=self,
            )
            self.current_page_index = PAGE_PLAN_REVIEW
        else:
    ...
    def _go_to_previous_page(self):
        ...
        if self.current_page_index == PAGE_PLAN_REVIEW:
            self.current_page_index = PAGE_GOAL_INPUT
            self.intermediate_plan = None
            self.profile_generator.intermediate_plan = None
    ...
    def _reset_and_load_step_state(self):
        self.ui_suggested_condition_for_step = None
        self.ui_suggested_action_for_step = None
        self.ui_element_to_refine_desc = None
        self.ui_refined_element_candidates = []
        self.ui_selected_candidate_box_index = None
        self.ui_confirmed_element_for_action = None
        ...
        if not self.current_plan_step_data:
            self.current_step_region_name = None
            self.current_step_region_coords = None
            self.current_step_region_defined_for_pg = False
            self.current_step_region_image_np = None
            self.current_step_region_image_pil_for_display = None
            self._temp_suggested_region_coords = None
        ...
        if self.current_step_region_name and self.current_step_region_coords:
            ...
            if self.current_step_region_defined_for_pg:
                if self.current_full_context_np is not None:
                    x, y, w, h = self.current_step_region_coords["x"], self.current_step_region_coords["y"], self.current_step_region_coords["width"], self.current_step_region_coords["height"]
                    img_h_f, img_w_f = self.current_full_context_np.shape[:2]
                    ...
                    if w_c > 0 and h_c > 0:
                        ...
                        self.current_step_region_image_np = self.current_step_region_image_pil_for_display = None
            else:
                self.current_step_region_image_np = None
                self.current_step_region_image_pil_for_display = None
        else:
        ...
        if found_rule_for_step:
            ...
            self.ui_element_to_refine_desc = None
        logger.debug(
            f"Wizard: State loaded for step {step_id_from_plan}. Region '{self.current_step_region_name}' defined in PG: {self.current_step_region_defined_for_pg}. Rule logic loaded: {found_rule_for_step is not None}"
        )
    ...
    def _setup_page_step_define_logic(self):
        ...
        main_logic_area = ctk.CTkFrame(page_frame, fg_color="transparent")
        main_logic_area.pack(fill="both", expand=True)
        ...
        self.btn_capture_template_for_step = ctk.CTkButton(element_interaction_frame, text="Use Template Instead", command=self._handle_capture_template_for_step, width=160, state="disabled")
        ...
        if self.ui_suggested_condition_for_step and self.ui_suggested_action_for_step:
            ...
            self._render_step_logic_editors(self.ui_suggested_condition_for_step, self.ui_suggested_action_for_step, self.ui_element_to_refine_desc)
        else:
            self.after(100, self._trigger_ai_logic_suggestion_for_step_threaded)

        self._update_navigation_buttons_state()
        logger.debug(f"Page Step Define Logic UI setup for step {step_id}.")
    ...
    def _trigger_ai_logic_suggestion_for_step_threaded(self):
        if not self.current_plan_step_data or self.current_step_region_image_np is None or not self.current_step_region_name:
            ...
            return
        self._show_loading_overlay("AI suggesting logic...")
        self.btn_refine_element.configure(state="disabled")
        self.btn_capture_template_for_step.configure(state="disabled")
    ...
    def _perform_ai_logic_suggestion_in_thread(self, plan_step_data: IntermediatePlanStep, region_image_np: np.ndarray, region_name: str):
        ...
        try:
            ...
            logger.error(f"Exception in AI logic suggestion thread: {e}", exc_info=True)
            error = e
    ...
    def _handle_ai_logic_suggestion_result(self, suggestion_result: Optional[Dict[str, Any]], error: Optional[Exception]):
        ...
        if error:
            ...
            self.ui_suggested_condition_for_step = copy.deepcopy(s_cond)
            self.ui_suggested_action_for_step = copy.deepcopy(s_act)
            self.ui_element_to_refine_desc = el_ref_desc
            messagebox.showinfo(
                "AI Logic Suggestion",
            ...
            messagebox.showwarning("AI Suggestion Failed", "AI could not suggest logic for this step. Please configure manually or use defaults.", parent=self)
    ...
    def _render_step_logic_editors(self, condition_data: Dict[str, Any], action_data: Dict[str, Any], element_to_refine_description: Optional[str]):
        ...
        cond_menu = ctk.CTkOptionMenu(
            self.step_logic_condition_frame, variable=cond_type_var, values=CONDITION_TYPES, command=lambda choice: self._on_wizard_logic_type_change("condition", choice)
        )  # Corrected: CONDITION_TYPES
        cond_menu.grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        self.step_logic_optionmenu_vars["step_cond_type_var"] = cond_type_var
        self.step_logic_detail_widgets["step_cond_type"] = cond_menu
        ...
        act_menu = ctk.CTkOptionMenu(
            self.step_logic_action_frame, variable=act_type_var, values=ACTION_TYPES, command=lambda choice: self._on_wizard_logic_type_change("action", choice)
        )  # Corrected: ACTION_TYPES
        act_menu.grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        self.step_logic_optionmenu_vars["step_act_type_var"] = act_type_var
        self.step_logic_detail_widgets["step_act_type"] = act_menu
        ...
        elif hasattr(self, "element_refine_entry"):
            self.element_refine_entry.delete(0, tk.END)
            self.btn_refine_element.configure(state="disabled")
    ...
    def _on_wizard_logic_type_change(self, part_key: str, new_type: str):
        ...
        if part_key == "condition":
            region_widget = self.step_logic_detail_widgets.get(f"{widget_prefix}region")
            region_var = self.step_logic_optionmenu_vars.get(f"{widget_prefix}region_var")
    ...
    def _render_dynamic_params_in_wizard_subframe(self, param_group_key: str, item_subtype: str, data_source: Dict[str, Any], parent_frame: ctk.CTkFrame, start_row: int, widget_prefix: str):
        for child_widget in list(parent_frame.winfo_children()):
            ...
            if grid_info and grid_info.get("row", -1) >= start_row:
                ...
                if widget_key_to_pop:
                    self.step_logic_detail_widgets.pop(widget_key_to_pop, None)
                    self.step_logic_optionmenu_vars.pop(f"{widget_key_to_pop}_var", None)
                child_widget.destroy()

        self.step_logic_param_widgets_and_defs.clear()
        self.step_logic_controlling_widgets.clear()
        self.step_logic_widget_prefix = widget_prefix
        ...
        for p_def in param_defs_for_subtype:
            ...
            if w_type == "entry":
                ...
                entry.bind("<KeyRelease>", lambda e: self.main_app_instance._set_dirty_status(True))
                ...
                textbox.bind("<FocusOut>", lambda e: self.main_app_instance._set_dirty_status(True))
                ...
                option_menu = ctk.CTkOptionMenu(parent_frame, variable=tk_var, values=options_list, command=lambda choice, p=p_def: self._update_step_logic_conditional_visibility(p, choice))
                self.step_logic_optionmenu_vars[f"{widget_full_key}_var"] = tk_var
                ...
                if any(other_pdef.get("condition_show", {}).get("field") == p_id for other_pdef in param_defs_for_subtype if other_pdef.get("condition_show")):
                    self.step_logic_controlling_widgets[p_id] = created_widget_instance
            ...
            if created_widget_instance:
                ...
                self.step_logic_param_widgets_and_defs.append({"widget": created_widget_instance, "label_widget": label_widget, "param_def": p_def})
                current_r += 1
            else:
    ...
    def _apply_step_logic_conditional_visibility(self):
        if not hasattr(self, "step_logic_param_widgets_and_defs") or not hasattr(self, "step_logic_controlling_widgets") or not hasattr(self, "step_logic_widget_prefix"):
            return
        for item in self.step_logic_param_widgets_and_defs:
            ...
            if visibility_config:
                ...
                controller_widget_instance = self.step_logic_controlling_widgets.get(controlling_field_id)
                current_controller_value = None
                if isinstance(controller_widget_instance, ctk.CTkOptionMenu):
                    tk_var_for_controller = self.step_logic_optionmenu_vars.get(f"{self.step_logic_widget_prefix}{controlling_field_id}_var")
    ...
    def _handle_ai_refine_element_threaded(self):
        ...
        if not element_desc_to_refine:
            messagebox.showwarning("Input Missing", "Enter element description to find.", parent=self)
            return
        if not self.current_step_region_image_np or not self.current_step_region_name:
            messagebox.showerror("Error", "Region image or name is missing for refinement.", parent=self)
            return
        self.ui_element_to_refine_desc = element_desc_to_refine
        self._show_loading_overlay(f"AI refining '{element_desc_to_refine[:30]}...'")
        self.btn_refine_element.configure(state="disabled")
    ...
    def _perform_ai_refine_element_in_thread(self, element_desc: str, region_image_np: np.ndarray, region_name: str):
        ...
        try:
            ...
            logger.error(f"Exception in AI refine element thread: {e}", exc_info=True)
            error = e
    ...
    def _handle_ai_refine_element_result(self, candidates: Optional[List[Dict[str, Any]]], error: Optional[Exception]):
        ...
        if error:
            messagebox.showerror("AI Error", f"Error during AI element refinement: {error}", parent=self)
            return
        self.ui_refined_element_candidates = candidates if candidates else []
        self.ui_selected_candidate_box_index = None
        self._clear_action_target_params()
        if self.ui_refined_element_candidates:
            self._display_current_step_region_image_with_candidates(self.ui_refined_element_candidates, None)
            messagebox.showinfo("AI Element Candidates", f"AI found {len(self.ui_refined_element_candidates)} candidate(s). Click one on the image to select for action.", parent=self)
        else:
            self._display_current_step_region_image_with_candidates(None, None)
            messagebox.showwarning("AI Refinement", "AI could not find matching elements, or refinement failed.", parent=self)
    ...
    def _handle_capture_template_for_step(self):
        if not self.current_step_region_image_pil_for_display or not self.current_step_region_name:
            messagebox.showerror("Error", "Region image not available for template capture.", parent=self)
            return
        ...
        while any(t.get("name") == suggested_tpl_name for t in self.profile_generator.generated_profile_data.get("templates", [])):
            count += 1
        ...
        if not template_name_input or not template_name_input.strip():
            logger.info("Template capture cancelled: No name.")
            return
        ...
        if any(t.get("name") == template_name for t in self.profile_generator.generated_profile_data.get("templates", [])):
            messagebox.showerror("Name Conflict", f"A template named '{template_name}' already exists in this draft.", parent=self)
            return
        ...
        if selected_template_coords_xywh:
            ...
            if self.current_step_region_image_np is not None and w_rel > 0 and h_rel > 0:
                ...
                template_metadata = {
                    ...
                    "comment": f"Template for '{element_desc_for_template}' in region '{self.current_step_region_name}'. User captured.",
                    "_image_data_np_for_save": template_image_np_bgr,
                }
    ...
    def _save_generated_profile(self):
        ...
        if not filename_base_from_ui:
            messagebox.showerror("Filename Missing", "Please enter a filename for the profile.", parent=self)
            return

        profile_data_for_saving = self.profile_generator.get_generated_profile_data()
        if self.user_goal_text:
            profile_data_for_saving["profile_description"] = f"AI-Generated for goal: {self.user_goal_text[:120]}"
        ...
        filepath_chosen_by_user = tk.filedialog.asksaveasfilename(
        ...
        if filepath_chosen_by_user:
            ...
            if success:
                ...
                messagebox.showerror("Save Failed", "Could not save profile or its templates. Please check logs for details.", parent=self)
        else:
            logger.info("Profile save dialog cancelled by user.")
