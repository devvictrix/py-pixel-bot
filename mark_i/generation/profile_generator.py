...
class ProfileGenerator:
    ...
    def start_profile_generation(
        ...
        if not isinstance(intermediate_plan, list) or not intermediate_plan:
            logger.error("ProfileGenerator: Cannot start: Intermediate plan empty/invalid.")
            return False
        ...
        if initial_full_screen_context_np is not None and isinstance(initial_full_screen_context_np, np.ndarray):
            ...
            logger.warning("ProfileGenerator: No initial visual context provided.")
    ...
    def set_current_visual_context(self, screen_capture_np: Optional[np.ndarray]):  # Unchanged from Phase 1 Impl
        if screen_capture_np is not None and isinstance(screen_capture_np, np.ndarray):
            ...
            logger.warning("PG: Invalid visual context (not NumPy array or None).")
    ...
    def advance_to_next_plan_step(self) -> Optional[IntermediatePlanStep]:  # Unchanged from Phase 1 Impl
        ...
        if self.current_plan_step_index < len(self.intermediate_plan) - 1:
            ...
            logger.info("PG: All plan steps processed.")
            return None
    ...
    def suggest_region_for_step(self, plan_step: IntermediatePlanStep) -> Optional[Dict[str, Any]]:
        ...
        if self.current_full_visual_context_np is None:
            logger.warning(f"{log_prefix}: No current visual context image.")
            return None
        if not step_description:
            logger.warning(f"{log_prefix}: Plan step description empty.")
            return None
        ...
        response = self.gemini_analyzer.query_vision_model(prompt=prompt, image_data=self.current_full_visual_context_np, model_name_override=DEFAULT_REGION_SUGGESTION_MODEL)  # Prompt first
    ...
    def suggest_logic_for_step(self, plan_step: IntermediatePlanStep, focused_region_image: np.ndarray, focused_region_name: str) -> Optional[Dict[str, Any]]:
        ...
        if focused_region_image is None:
            logger.warning(f"{log_prefix}: No focused region image.")
            return None
        if not step_description:
            logger.warning(f"{log_prefix}: Plan step description empty.")
            return None
            ...
            try:
                ...
                if isinstance(content, dict) and isinstance(content.get("suggested_condition"), dict) and isinstance(content.get("suggested_action"), dict):
                    ...
                    if s_act.get("type") == "click":
                        s_act.pop("target_region", None)  # Click target related to element_to_refine
    ...
    def refine_element_location(  # Unchanged from previous full version
        ...
        if not element_description:
            logger.warning(f"{log_prefix}: Element description empty.")
            return None
        prompt = (
            f"Precise visual element locator. In image of region '{focused_region_name}', identify all distinct elements matching: \"{element_description}\".\n"
                ...
                if isinstance(content, dict) and isinstance(content.get("elements"), list):
                    for elem_data in content["elements"]:
                        if (
                            ...
                            candidates.append(
                                {"box": box, "label_suggestion": elem_data.get("label_suggestion", element_description), "confidence": elem_data.get("confidence", 1.0)}
                            )  # GUI needs "box"
                    if candidates:
                        logger.info(f"{log_prefix}: Refined to {len(candidates)} candidates. First: {candidates[0]['box']}")
                    else:
                        logger.info(f"{log_prefix}: Refinement found no matching elements or box data invalid.")
    ...
    def add_region_definition(self, region_data: Dict[str, Any]) -> bool:  # Unchanged
        if not (isinstance(region_data, dict) and region_data.get("name") and all(isinstance(region_data.get(k), int) for k in ["x", "y", "width", "height"])):
            logger.error(f"PG: Invalid region data to add: {region_data}")
            return False
        final_data = {**copy.deepcopy(DEFAULT_REGION_STRUCTURE_PG), **region_data}
        self.generated_profile_data.setdefault("regions", []).append(final_data)
        logger.info(f"PG: Region '{final_data['name']}' added to draft.")
        return True
    ...
    def add_template_definition(self, template_data: Dict[str, Any]) -> bool:  # Unchanged
        if not (isinstance(template_data, dict) and template_data.get("name") and template_data.get("filename")):
            logger.error(f"PG: Invalid template data: {template_data}")
            return False
        ...
        logger.info(f"PG: Template '{final_data['name']}' added to draft.")
        return True
    ...
    def add_rule_definition(self, rule_data: Dict[str, Any]) -> bool:  # Unchanged
        if not (isinstance(rule_data, dict) and rule_data.get("name") and isinstance(rule_data.get("condition"), dict) and isinstance(rule_data.get("action"), dict)):
            logger.error(f"PG: Invalid rule data: {rule_data}")
            return False
        final_data = copy.deepcopy(DEFAULT_RULE_STRUCTURE_PG)
        final_data.update(rule_data)
        ...
        logger.info(f"PG: Rule '{final_data['name']}' added to draft.")
        return True
    ...
    def save_generated_profile(self, filepath_to_save: str) -> bool:  # Unchanged
        if not self.generated_profile_data:
            logger.error("PG: No profile data to save.")
            return False
        if not filepath_to_save:
            logger.error("PG: No filepath to save profile.")
            return False
        try:
            self.config_manager.update_profile_data(self.generated_profile_data)
            return self.config_manager.save_current_profile(filepath_to_save)
        except Exception as e:
            logger.error(f"PG: Failed to save generated profile to '{filepath_to_save}': {e}", exc_info=True)
            return False
