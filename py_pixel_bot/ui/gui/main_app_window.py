...
class MainAppWindow(ctk.CTk):
    ...
    def _load_profile_from_path(self, fp: str):
        try:
            ...
            if not self.profile_data:
                raise ValueError("Loaded profile data is empty.")
    ...
    def _populate_ui_from_profile_data(self):
        ...
        self._populate_specific_list_frame(
            ...
            lambda i, idx: f"{i.get('name',f'T{idx+1}')} ({i.get('filename','N/A')})",
    ...
    def _on_item_selected(self, list_name: str, item_data: Dict, item_index: int, item_widget_frame: Optional[ctk.CTkFrame]):
        if not item_widget_frame:
            logger.warning(f"Item selection called for {list_name} but item_widget_frame is None. Aborting selection.")
            return
        ...
        if btn_to_enable:
            btn_to_enable.configure(state="normal")
    ...
    def _highlight_selected_list_item(self, list_name_for_attr: str, new_selected_widget: Optional[ctk.CTkFrame], is_sub_list: bool = False):
        ...
        old_widget_attr_target = self.details_panel_instance if is_sub_list and self.details_panel_instance else self
    ...
    def _apply_region_changes(self):
        ...
        self.details_panel_instance.update_display(current_region_data, "region")
    ...
    def _apply_template_changes(self):
        ...
        self._set_dirty_status(True)
        self._populate_specific_list_frame(
            "template", self.templates_list_scroll_frame, self.profile_data["templates"], lambda i, idx: f"{i.get('name')} ({i.get('filename')})", self.btn_remove_template, "template"
        )
    ...
    def _apply_rule_changes(self):
        ...
        temp_rule_data["region"] = rule_region_var.get() if rule_region_var else ""
        ...
        if is_compound:
            ...
            new_cond_data["logical_operator"] = log_op_var.get() if log_op_var else "AND"

            new_sub_conds = []
            # Use the sub-conditions from the temp_rule_data (which is a deepcopy of the current rule's state)
            # because the DetailsPanel might have added/removed sub-conditions that are reflected in its UI
            # but not yet in temp_rule_data's direct 'condition.sub_conditions' if we only relied on that.
            # The _get_parameters_from_ui will read from the widgets that DetailsPanel has rendered.
            # However, for non-edited sub-conditions, we need the existing data.

            profile_sub_conds_list = temp_rule_data.get("condition", {}).get("sub_conditions", [])

            all_subs_valid = True
            for idx, sub_c_profile_data in enumerate(profile_sub_conds_list):
            ...
            if not all_subs_valid:
                logger.error("Sub-condition validation failed. Rule not saved.")
                return
            ...
            if single_cond_params is None:
                logger.error("Single condition validation failed. Rule not saved.")
                return
        ...
        if action_params is None:
            logger.error("Action validation failed. Rule not saved.")
            return
    ...
    def _edit_region_coordinates_with_selector(self):
        if self.selected_region_index is None:
            return
        if self._is_dirty:
            if not messagebox.askyesno("Save Changes?", "Save current profile changes before launching Region Selector?"):
                return
    ...
    def _add_region(self):
        if not self.current_profile_path:
            if not messagebox.askokcancel("Save Required", "Profile must be saved. Save now?"):
                return
            if not self._save_profile_as():
                return
        if self._is_dirty:
            if not messagebox.askyesno("Save Changes?", "Save current changes before Region Selector?"):
                return
            if not self._save_profile():
                return
        try:
            ...
            messagebox.showerror("Add Region Error", f"Error with Region Selector:\n{e}")
    ...
    def _remove_selected_item(self, list_name_key: str, profile_data_key: str, selected_index_attr: str, remove_button_widget: Optional[ctk.CTkButton]):
        ...
        if 0 <= selected_index < len(item_list):
            ...
            display_cb_map = {
                ...
                "template": lambda i, idx: f"{i.get('name', 'T_NoName')} ({i.get('filename', 'F_NoName')})",
            ...
            if list_name_key == "template" and self.current_profile_path:
                ...
                if fn_del:
                    ...
                    try:
                        ...
                        messagebox.showwarning("File Deletion Error", f"Could not delete '{fn_del}':\n{e_os.strerror}")
        else:
    ...
    def _add_template(self):
        if not self.current_profile_path:
            ...
            if not self._save_profile_as():
                return
        if self._is_dirty:
            if not messagebox.askyesno("Save Changes?", "Save current changes before adding template?"):
                return
            if not self._save_profile():
                return
        ...
        if any(t.get("name") == tpl_name for t in self.profile_data.get("templates", [])):
            messagebox.showerror("Name Error", f"Template name '{tpl_name}' already exists.")
            return
        ...
        try:
            ...
            while os.path.exists(tp):
                tf = f"{sane_base}_{c}{ext}"
                tp = os.path.join(templates_dir, tf)
                c += 1
            shutil.copy2(img_path, tp)
            new_tpl = {"name": tpl_name, "filename": tf}
            self.profile_data.setdefault("templates", []).append(new_tpl)
            self._populate_specific_list_frame(
                "template",
                self.templates_list_scroll_frame,
                self.profile_data["templates"],
                lambda i, idx: f"{i.get('name', 'T_NoName')} ({i.get('filename', 'F_NoName')})",
                self.btn_remove_template,
                "template",
            )
            self._set_dirty_status(True)
            messagebox.showinfo("Template Added", f"Template '{tpl_name}' added.")
        except Exception as e:
            messagebox.showerror("Add Template Error", f"Could not add template:\n{e}")

    def _add_new_rule(self):
        if not self.profile_data:
            self._new_profile(prompt_save=False)
        name_dialog = ctk.CTkInputDialog(text="Unique name for new rule:", title="New Rule Name")
        ...
        if any(r.get("name") == rule_name for r in self.profile_data.get("rules", [])):
            messagebox.showerror("Name Error", f"Rule name '{rule_name}' already exists.")
            return
        ...
        self._populate_specific_list_frame("rule", self.rules_list_scroll_frame, self.profile_data["rules"], lambda i, idx: i.get("name", f"Rule{idx+1}"), self.btn_remove_rule, "rule")
        self._set_dirty_status(True)
        messagebox.showinfo("Rule Added", f"Rule '{rule_name}' added.")
    ...
    def _on_rule_part_type_change(self, part_changed: str, new_type_selected: str):
        ...
        if part_changed == "condition":
            ...
            condition_data_source: Optional[Dict] = None
            widget_prefix = ""
            ...
            if self.selected_sub_condition_index is not None:
                ...
                if 0 <= self.selected_sub_condition_index < len(sub_cond_list):
                    condition_data_source = sub_cond_list[self.selected_sub_condition_index]
                ...
                condition_data_source = current_rule_data.get("condition", {})
                target_frame_for_params = self.details_panel_instance.condition_params_frame
                widget_prefix = "cond_"

            if target_frame_for_params and condition_data_source is not None:
                if condition_data_source.get("type") != new_type_selected:
                    preserved_data = {"type": new_type_selected}
                    if widget_prefix == "subcond_":
                        if "region" in condition_data_source:
                            preserved_data["region"] = condition_data_source["region"]
                        if "capture_as" in condition_data_source:
                            preserved_data["capture_as"] = condition_data_source["capture_as"]
                    condition_data_source.clear()
                    condition_data_source.update(preserved_data)
                else:
                    condition_data_source["type"] = new_type_selected
                self.details_panel_instance._render_dynamic_parameters("conditions", new_type_selected, condition_data_source, target_frame_for_params, start_row=1, widget_prefix=widget_prefix)
            ...
            if self.details_panel_instance and self.details_panel_instance.action_params_frame:
                self.details_panel_instance._render_dynamic_parameters(
                    "actions", new_type_selected, action_to_update, self.details_panel_instance.action_params_frame, start_row=1, widget_prefix="act_"
                )
    ...
    def _add_sub_condition_to_rule(self):
        ...
        self._set_dirty_status(True)
        self.details_panel_instance.update_display(rule, "rule")
    ...
    def _convert_condition_structure(self):
        ...
        if is_compound:
            ...
            if len(subs) > 1 and not messagebox.askyesno("Confirm", "Convert to single? First sub-cond kept. Others lost. Continue?"):
                return
            rule["condition"] = copy.deepcopy(subs[0]) if subs else {"type": "always_true"}
        else:
            single = copy.deepcopy(cond)
            if not single.get("type"):
                single = {"type": "always_true"}
