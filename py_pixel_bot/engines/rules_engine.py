...
import os
import re 
from typing import Dict, List, Any, Optional, Tuple, Set # Added Set
...
class RulesEngine:
    ...
    def _parse_rule_analysis_dependencies(self):
        ...
        for i, rule in enumerate(self.rules):
            ...
            if "logical_operator" in condition_spec and "sub_conditions" in condition_spec and \
               isinstance(condition_spec["sub_conditions"], list):
                ...
                for sub_idx, sub_cond_spec in enumerate(condition_spec["sub_conditions"]):
                    ...
                    if sub_cond_region: # Region must be defined for dependency to be meaningful
                        conditions_to_evaluate_for_deps.append(
                            (sub_cond_spec, sub_cond_region, f"{rule_name}/Sub#{sub_idx+1}")
                        )
                    else:
            ...
            for cond_spec_item, region_name_item, cond_ctx_name_item in conditions_to_evaluate_for_deps:
                ...
                if cond_type == "ocr_contains_text": required_analysis = "ocr"
                elif cond_type == "dominant_color_matches": required_analysis = "dominant_color"
                elif cond_type == "average_color_is": required_analysis = "average_color"
                ...
                if required_analysis and region_name_item: # Must have a region to associate with
                    ...
                    logger.debug(
                        f"Dependency parser: Condition '{cond_ctx_name_item}' (type: {cond_type}) in region '{region_name_item}' "
                        f"requires pre-emptive analysis: '{required_analysis}'."
                    )
    ...
    def _load_template_image_for_rule(self, template_filename: str, rule_name_for_context: str) -> Optional[np.ndarray]:
        ...
        try:
            if not os.path.exists(template_full_path):
                ...
                self._loaded_templates[cache_key] = None; return None
                    ...
                    if dot_path_str: # Accessing keys in a dictionary
                        ...
                        try:
                            for key_part in keys:
                                if isinstance(current_value, dict): current_value = current_value[key_part]
                                elif isinstance(current_value, list) and key_part.isdigit(): current_value = current_value[int(key_part)] # Basic list index access
                                else: logger.warning(f"R '{rule_name_for_context}', Subst: Cannot access key '{key_part}' in non-dict/list or invalid index for var '{var_name}'. Placeholder '{full_placeholder}' left."); return full_placeholder
                            str_value = str(current_value)
                            logger.debug(f"R '{rule_name_for_context}', Subst: Replaced '{full_placeholder}' with value '{str_value}'.")
                            return str_value
                        except (KeyError, IndexError): logger.warning(f"R '{rule_name_for_context}', Subst: Path '{dot_path_str}' not found for var '{var_name}'. Placeholder '{full_placeholder}' left."); return full_placeholder
                        except Exception as e: logger.warning(f"R '{rule_name_for_context}', Subst: Error accessing path '{dot_path_str}' for var '{var_name}': {e}. Placeholder left."); return full_placeholder
                    else: str_value = str(current_value); logger.debug(f"R '{rule_name_for_context}', Subst: Replaced '{full_placeholder}' with value '{str_value}'."); return str_value
                else: logger.warning(f"R '{rule_name_for_context}', Subst: Var '{var_name}' for '{full_placeholder}' not found. Placeholder left."); return full_placeholder
            return PLACEHOLDER_REGEX.sub(replace_match, input_value)
        elif isinstance(input_value, list): return [self._substitute_variables(item, variable_context, rule_name_for_context) for item in input_value]
        elif isinstance(input_value, dict): return {key: self._substitute_variables(value, variable_context, rule_name_for_context) for key, value in input_value.items()}
        else: return input_value


    def _evaluate_single_condition_logic(
        self, single_condition_spec: Dict[str, Any], region_name: str, region_data: Dict[str, Any], 
        rule_name_for_context: str, variable_context: Dict[str, Any]
    ) -> bool:
        condition_type = single_condition_spec.get("type")
        capture_as_var_name = single_condition_spec.get("capture_as")
        if not condition_type: logger.error(f"R '{rule_name_for_context}', Rgn '{region_name}': Cond 'type' missing."); return False
        ...
        condition_met = False; captured_value_for_var = None
        ...
        def get_analysis_data_with_fallback(data_key: str, analysis_func: Callable, *args_for_analysis_func) -> Any:
            ...
            if data is None and captured_image is not None:
                ...
                try: data = analysis_func(*args_for_analysis_func)
                except Exception as e_fb: logger.error(f"R '{rule_name_for_context}', Fallback calc for '{data_key}' failed: {e_fb}"); data = None
        ...
        try:
            if condition_type == "pixel_color":
                if captured_image is None: logger.warning(f"R '{rule_name_for_context}', T 'pixel_color': Image is None."); return False
                # ... (validation of params, then call self.analysis_engine.analyze_pixel_color - same as before) ...
                rx=single_condition_spec.get("relative_x");ry=single_condition_spec.get("relative_y");ebgr=single_condition_spec.get("expected_bgr");tol=single_condition_spec.get("tolerance",0)
                if not all(isinstance(v,int) for v in [rx,ry]) or not (isinstance(ebgr,list) and len(ebgr)==3 and all(isinstance(c,int)for c in ebgr)): logger.error(f"R '{rule_name_for_context}',T 'pixel_color': Invalid params."); return False
                ...
                if avg_color_data is None: return False
                # ... (rest of average_color_is logic - same as before) ...
                ebgr=single_condition_spec.get("expected_bgr");tol=single_condition_spec.get("tolerance",10)
                if not(isinstance(ebgr,list)and len(ebgr)==3 and all(isinstance(c,int)for c in ebgr)):logger.error(f"R '{rule_name_for_context}',T 'avg_color': Invalid expected_bgr.");return False
                c_diff=np.abs(np.array(avg_color_data)-np.array(ebgr));condition_met=np.all(c_diff<=tol)
                ...
                if captured_image is None: logger.warning(f"R '{rule_name_for_context}', T 'template': Image is None."); return False
                # ... (rest of template_match_found, including capture - same as before) ...
                tpl_fn=single_condition_spec.get("template_filename");min_conf=single_condition_spec.get("min_confidence",0.8)
                if not tpl_fn: logger.error(f"R '{rule_name_for_context}',T 'template': Missing filename.");return False
                tpl_img=self._load_template_image_for_rule(tpl_fn,rule_name_for_context)
                if tpl_img is None: return False
                match_res=self.analysis_engine.match_template(captured_image,tpl_img,min_conf)
                if match_res:
                    condition_met=True;self._last_template_match_info={"found":True,**match_res,"matched_region_name":region_name}
                    logger.info(f"R '{rule_name_for_context}',T 'template':'{tpl_fn}' FOUND conf {match_res['confidence']:.2f}.")
                    if capture_as_var_name:captured_value_for_var={"x":match_res["location"][0],"y":match_res["location"][1],"width":match_res["width"],"height":match_res["height"],"confidence":match_res["confidence"]}
                else:self._last_template_match_info={"found":False};condition_met=False;logger.debug(f"R '{rule_name_for_context}',T 'template':'{tpl_fn}' NOT found.")
                ...
                if not ocr_res or "text" not in ocr_res: return False # If no text after fallback, condition fails
                # ... (rest of ocr_contains_text, including capture - same as before) ...
                actual_txt=ocr_res.get("text","");actual_conf=ocr_res.get("average_confidence",0.0);txt_find=single_condition_spec.get("text_to_find");cs=single_condition_spec.get("case_sensitive",False);min_ocr_conf=single_condition_spec.get("min_ocr_confidence")
                if not txt_find:logger.error(f"R '{rule_name_for_context}',T 'ocr': Missing text_to_find.");return False
                txts_check=[txt_find]if isinstance(txt_find,str)else txt_find;proc_ocr_txt=actual_txt if cs else actual_txt.lower();txt_match=False
                for ti in txts_check:
                    proc_ti=ti if cs else ti.lower()
                    if proc_ti in proc_ocr_txt:txt_match=True;break
                if not txt_match:logger.debug(f"R '{rule_name_for_context}',OCR: Text '{txt_find}' NOT found (Conf:{actual_conf:.1f}%).");return False
                conf_crit_met=True
                if min_ocr_conf is not None:conf_crit_met=actual_conf>=float(min_ocr_conf)
                if conf_crit_met:condition_met=True;logger.info(f"R '{rule_name_for_context}',OCR:MET. Text '{txt_find}' found (Conf:{actual_conf:.1f}%).");
                if capture_as_var_name and condition_met:captured_value_for_var=actual_txt # Capture full text if all criteria met
                else:condition_met=False;logger.info(f"R '{rule_name_for_context}',OCR:NOT MET. Text found, but conf {actual_conf:.1f}% < {min_ocr_conf}%.")
                ...
                dom_color_res = get_analysis_data_with_fallback(
                    ...
                    captured_image, 
                    self.config_manager.get_setting("analysis_dominant_colors_k", 3), # Get K from settings
                    region_name
                )
                if not dom_color_res: return False # If no dominant colors after fallback
                # ... (rest of dominant_color_matches - same as before) ...
                ebgr=single_condition_spec.get("expected_bgr");tol=single_condition_spec.get("tolerance",10);top_n=single_condition_spec.get("check_top_n_dominant",1);min_perc=single_condition_spec.get("min_percentage",0.0)
                if not ebgr:logger.error(f"R '{rule_name_for_context}',T DomColor:Missing expected_bgr.");return False
                n_check=min(top_n,len(dom_color_res));
                if n_check==0 and top_n>0:logger.debug(f"R '{rule_name_for_context}',T DomColor:No dom colors to check.");return False
                for i in range(n_check):
                    dci=dom_color_res[i];act_bgr,act_p=dci["bgr_color"],dci["percentage"]
                    cmatch=np.all(np.abs(np.array(act_bgr)-np.array(ebgr))<=tol);pmatch=act_p>=min_perc
                    logger.debug(f"R '{rule_name_for_context}',DomColorChk#{i+1}:Actual {act_bgr}(Perc {act_p:.1f}%).Exp {ebgr}.CMatch:{cmatch}.PMatch:{pmatch}.")
                    if cmatch and pmatch:condition_met=True;logger.info(f"R '{rule_name_for_context}',T DomColor:MET.Dom BGR {act_bgr}(Perc {act_p:.1f}%) matched.");break
                if not condition_met:logger.info(f"R '{rule_name_for_context}',T DomColor:NOT MET.")

            elif condition_type == "always_true": condition_met = True; logger.debug(f"R '{rule_name_for_context}', T 'always_true': Met.")
            else: logger.error(f"R '{rule_name_for_context}', Rgn '{region_name}': Unknown cond type '{condition_type}'."); return False
            ...
            if condition_met and capture_as_var_name and captured_value_for_var is not None:
                ...
                val_snip = str(captured_value_for_var); val_snip = (val_snip[:67] + '...') if len(val_snip) > 70 else val_snip
        ...
        except Exception as e: logger.exception(f"R '{rule_name_for_context}', Rgn '{region_name}', T '{condition_type}': Exception: {e}"); return False
    ...
    def _check_condition(
        self, rule_name: str, condition_spec: Dict[str, Any], default_rule_region: Optional[str], 
        all_region_data: Dict[str, Dict[str, Any]], variable_context: Dict[str, Any]
        ...
        logical_operator_str = condition_spec.get("logical_operator"); sub_conditions_original = condition_spec.get("sub_conditions")
        if logical_operator_str and isinstance(sub_conditions_original, list):
            operator = logical_operator_str.upper()
            if operator not in ["AND", "OR"]: logger.error(f"R '{rule_name}': Invalid op '{logical_operator_str}'."); return False
            if not sub_conditions_original: logger.warning(f"R '{rule_name}': Compound op '{operator}' no subs. False."); return False
            ...
            for i, sub_cond_orig_spec in enumerate(sub_conditions_original):
                ...
                if not sub_cond_region_name: logger.error(f"R '{ctx_name}': Region not specified/no default. Spec: {sub_cond_spec_subst}"); sub_result = False
                elif sub_cond_region_name not in all_region_data: logger.error(f"R '{ctx_name}': Region '{sub_cond_region_name}' not in all_region_data (keys: {list(all_region_data.keys())})."); sub_result = False
                else:
                ...
                if operator == "AND" and not sub_result: logger.info(f"R '{rule_name}': AND short-circuited False by Sub#{i+1}."); return False
                if operator == "OR" and sub_result: logger.info(f"R '{rule_name}': OR short-circuited True by Sub#{i+1}."); return True
            if operator == "AND": final_res = all(all_sub_results)
            elif operator == "OR": final_res = any(all_sub_results)
            else: final_res = False # Should not happen
            ...
            if not default_rule_region: logger.error(f"R '{rule_name}': Single cond, no default region. Spec: {condition_spec_subst}"); return False
            if default_rule_region not in all_region_data: logger.error(f"R '{rule_name}': Default region '{default_rule_region}' not in all_region_data (keys: {list(all_region_data.keys())})."); return False
    ...
    def evaluate_rules(self, all_region_data: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if not self.rules: logger.debug("No rules to evaluate."); return triggered_actions_specs
        logger.info(f"Starting evaluation of {len(self.rules)} rules.")
        for rule in self.rules:
            rule_name = rule.get("name", "UnnamedRule"); original_condition_spec = rule.get("condition")
            original_action_spec = rule.get("action"); default_rule_region = rule.get("region")
            if not original_condition_spec or not original_action_spec: logger.warning(f"R '{rule_name}': Missing cond/action. Skipping."); continue
            self._last_template_match_info = {"found": False}; rule_variable_context: Dict[str, Any] = {}
            ...
            try:
                ...
                if condition_is_met: # Overall condition result already logged by _check_condition or its delegates
                    ...
                    action_context = { "rule_name": rule_name, "condition_region": default_rule_region, "last_match_info": self._last_template_match_info.copy(), "variables": rule_variable_context.copy() }
                    ...
                    if action_target_region_name:
                        ...
                        if not action_context["target_region_config"]: logger.warning(f"R '{rule_name}', Action '{action_spec_subst.get('type')}': target_region '{action_target_region_name}' not in profile regions.")
            ...
            except Exception as e: logger.exception(f"R '{rule_name}': Unexpected error during rule eval/action prep: {e}")
