...
*   **FR-CORE-002: Configuration Files (JSON):** The system MUST use JSON for saving and loading bot configurations (profiles). These profiles MUST store:
    ...
    *   A list of rules, where each rule includes:
        ...
        *   A condition block, which can be:
            *   A single condition with a `type` (e.g., "pixel_color", "ocr_contains_text", **"gemini_vision_query" (v4.0.0)**) and specific parameters relevant to that type.
...
*   **FR-ANALYZE-007: Gemini Vision Query (v4.0.0):** The system MUST provide a condition type (`gemini_vision_query`) that allows rules to:
    ...
    *   Optionally specify a Gemini model name to override the default.
    *   Evaluate the condition based on the Gemini API's response:
        *   Check if the returned text content contains specific keyword(s) (case-sensitive or insensitive).
        *   **(Optional) Check if a value extracted from the returned JSON content (using JSONPath) matches an expected value.**
    *   Capture the Gemini API's text response or extracted JSON value into a rule-scoped variable using the `capture_as` parameter.
...
*   **FR-UI-003: Full GUI Profile Editor (`MainAppWindow` - v3.0.0 Target, plus v4.0.0 additions):**
    ...
    *   **FR-UI-003.2: Settings Editing:** The GUI MUST display and allow editing of general profile settings (e.g., "profile_description", "monitoring_interval_seconds", "analysis_dominant_colors_k", **"gemini_default_model_name" (v4.0.0)**). **(v4.0.0) It SHOULD display the status of the Gemini API key (e.g., "Loaded" or "Not found in .env").**
    ...
    *   **FR-UI-003.5: Rule Management & Editing (Comprehensive):**
        ...
        *   **Condition Editing:**
            ...
            *   **For single conditions and sub-conditions:** Allow selection of condition `type` (including **`gemini_vision_query` (v4.0.0)**) from a dropdown. Dynamically display and allow editing of all relevant parameters for the selected type using appropriate input widgets (**including multi-line text box for Gemini prompts**).
...
*   **FR-UI-004: User Feedback (General):** The system (CLI and GUI) MUST provide clear feedback about its operations, successes, warnings, and errors. **(v4.0.0) This includes feedback related to Gemini API interactions (e.g., errors during condition evaluation, API key status).**
