...
*   **FR-CORE-002: Configuration Files (JSON):** The system MUST use JSON for saving and loading bot configurations (profiles). These profiles MUST store:
    ...
    *   A list of rules, where each rule includes:
        ...
        *   An action block with a `type` (e.g., "click", "type_text", **"gemini_perform_task" (v4.0.0 Phase 2)**) and specific parameters relevant to that type, which can use captured variables via placeholder substitution.
...
*   **FR-CORE-004: Logging:** The system MUST implement comprehensive and persistent logging for all significant operations, decisions, errors, and state changes in both the backend engine and the GUI editor. Log levels and destinations MUST be configurable based on `APP_ENV` and CLI flags, as detailed in `adrs/ADR-007-Logging-Strategy-and-Environment-Configuration.md`.
    *   **(v4.0.0 - Phase 1 & 1.5):** Logging MUST include details of Gemini API interactions for `gemini_vision_query` (requests including prompt summaries and model used, response summaries including status and latency, errors).
    *   **(v4.0.0 - Phase 2):** Logging MUST extend to cover the `GeminiDecisionModule`'s operations, including input goals, prompts to Gemini for action suggestions, Gemini's raw suggestions, validation outcomes, refinement steps, and final executed actions or failure reasons.
...
*   **FR-ANALYZE-007: Gemini Vision Query (v4.0.0 Phase 1+):** The system MUST provide a condition type (`gemini_vision_query`) that allows rules to:
    ...
    *   Evaluate the condition based on the Gemini API's response:
        ...
        *   Check if a value extracted from the returned JSON content (using basic dot-notation path, e.g., `data.items.0.name`) matches an expected string value.
    *   Capture the Gemini API's text response, or the **entire parsed JSON object (if JSON response)**, or a specific value extracted from its `json_content` (if `expected_response_json_path` is used for extraction), into a rule-scoped variable using the `capture_as` parameter. This variable will be a dictionary `{ "value": <captured_data>, "_source_region_for_capture_": "region_name_used_for_gemini_query" }`.
    *   Handle API errors gracefully (condition evaluates to `False`, error logged).
*   **FR-ANALYZE-008: Gemini-Informed Action Suggestion (v4.0.0 Phase 2):** The system MUST provide a mechanism (`GeminiDecisionModule`) to:
    *   Accept a user-defined high-level goal or task description (string) and references to relevant screen regions for visual context.
    *   Utilize the `GeminiAnalyzer` to send this goal and visual context to a configured Google Gemini API endpoint.
    *   Prompt Gemini to suggest a single, simple action from a predefined, limited set of allowed actions (e.g., click described element, type in described field, press key). The suggestion should ideally be in a structured format (e.g., JSON) specifying action type and parameters.
    *   Parse and validate Gemini's suggested action against the allowed set and ensure parameters are sensible.
    *   If the suggested action involves targeting a vaguely described UI element, use `GeminiAnalyzer` again to obtain precise bounding box coordinates for that element based on its description and visual context.
    *   Provide status feedback on the decision-making process (e.g., success, failure, reason).
...
*   **FR-ACTION-001: Mouse Click Simulation:** The system MUST be able to simulate mouse clicks (left, right, middle, configurable number of clicks, interval between clicks) at:
    ...
    *   **(v4.0.0 - Phase 1.5)** Coordinates derived from bounding box data captured from a `gemini_vision_query` condition (e.g., center or top-left of element, via `gemini_element_variable` parameter).
...
*   **FR-ACTION-005: Dynamic Action Parameters (Variables):** Action parameters (e.g., text to type, X/Y coordinates for clicks, parts of log messages) MUST be able to utilize values captured into rule-scoped variables from conditions using a placeholder substitution mechanism (e.g., `{my_var}` or `{my_var.value.some_key}`). `ActionExecutor` must attempt to convert substituted string values to appropriate types for PyAutoGUI.
...
*   **FR-ACTION-007: Execute Gemini-Suggested Actions (v4.0.0 Phase 2):** The `ActionExecutor` MUST be callable by the `GeminiDecisionModule` to perform actions that Gemini has suggested and the `GeminiDecisionModule` has validated and refined (e.g., converting a described target into specific coordinates for a click). The set of actions executable this way will be from a predefined, limited list controlled by `GeminiDecisionModule`.
*   **FR-ACTION-008: User Confirmation for Gemini Actions (v4.0.0 Phase 2):** The system (likely via `GeminiDecisionModule`) SHOULD provide a configurable option (profile-level or task-level) to require user confirmation via a dialog before executing an action suggested by Gemini. This is primarily a safety and usability feature.
...
*   **FR-CONFIG-004: Gemini Task Definition (v4.0.0 Phase 2):** Users MUST be able to define "Gemini Tasks" within their profiles, likely as a new action type (e.g., `gemini_perform_task`) within a rule. This definition MUST allow specifying:
    *   A `goal_prompt` (text describing the task for Gemini).
    *   A list of `context_region_names` (names of defined regions in the profile to provide as visual input to Gemini for this task).
    *   Optionally, an `allowed_actions_override` list (a subset of system-wide predefined safe actions that Gemini can suggest for this specific task).
    *   Optionally, `max_steps` for simple sequential tasks (future iteration of Phase 2).
    *   Optionally, a boolean flag `require_confirmation_before_each_step` to override any global default for user confirmation for this specific task.
...
*   **FR-UI-001: CLI Control:** The system MUST provide Command Line Interface (CLI) controls for:
    ...
    *   **(v4.0.0 Phase 2 Consideration):** Potentially a new CLI option or command to run a profile in a "task-driven mode" if this mode is distinct from standard rule execution.
...
*   **FR-UI-003: Full GUI Profile Editor (`MainAppWindow` - with v4.0.0 Phase 2 additions):**
    ...
    *   **FR-UI-003.2: Settings Editing:** The GUI MUST display and allow editing of general profile settings (e.g., "profile_description", "monitoring_interval_seconds", "analysis_dominant_colors_k", "gemini_default_model_name" (v4.0.0 Phase 1+)). It MUST display the status of the Gemini API key (read-only).
    ...
    *   **FR-UI-003.5: Rule Management & Editing (Comprehensive):**
        ...
        *   **Condition Editing:** Allow selection of condition `type` (including `gemini_vision_query`), dynamic parameter editing, and conversion between single/compound conditions.
        *   **Action Editing:** Allow selection of action `type` (including **`gemini_perform_task` (v4.0.0 Phase 2)**). Dynamically display and allow editing of all relevant parameters for the selected type (including conditional parameters for `gemini_element_variable` for click actions, and parameters specific to `gemini_perform_task` such as `goal_prompt`, `context_region_names` via a multi-select mechanism, etc.).
...
*   **FR-UI-004: User Feedback (General - with v4.0.0 Phase 2 additions):** The system (CLI and GUI) MUST provide clear feedback about its operations, successes, warnings, and errors.
    *   **(v4.0.0 Phase 1+):** This includes feedback related to Gemini API interactions for `gemini_vision_query` (e.g., errors during condition evaluation, API key status in GUI).
    *   **(v4.0.0 Phase 2):** If the `GeminiDecisionModule` is active and `require_confirmation_before_each_step` is enabled, the GUI MUST display a confirmation dialog showing Gemini's proposed action before execution. Logs must detail the `GeminiDecisionModule`'s process.